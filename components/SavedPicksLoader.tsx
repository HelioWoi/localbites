...
import SavedPicksPage from '../screens/SavedPicksPage';
import { Loader2 } from 'lucide-react';

interface SavedPicksLoaderProps {
  slug: string;
}

const SavedPicksLoader: React.FC<SavedPicksLoaderProps> = ({ slug }) => {
  ...
  useEffect(() => {
    const loadRestaurant = async () => {
      try {
        ...
        const savedKey = `saved_dishes_${partnerData.id}`;
  ...
  if (loading) {
    return (
      <div className="h-screen w-screen bg-white flex flex-col items-center justify-center">
        ...
        <p className="text-zinc-400 text-sm">Loading your picks...</p>
      </div>
    );
  }
  ...
  if (error || !restaurant) {
    return (
      <div className="h-screen w-screen bg-white flex flex-col items-center justify-center p-8 text-center">
        ...
        <h1 className="text-zinc-900 text-2xl font-bold mb-2">Not Found</h1>
  ...
  return <SavedPicksPage restaurant={restaurant} />;
};

export default SavedPicksLoader;
