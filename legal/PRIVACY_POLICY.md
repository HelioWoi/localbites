# Privacy Policy

**Last Updated:** February 5, 2026

**MenuLove**  
ABN: 33 234 268 637  
Email: contact@menulove.com.au  
Address: 16 Smith Street, Mooloolaba, Sunshine Coast, Queensland, Australia

---

## 1. Introduction

MenuLove ("we", "our", "us") is committed to protecting your privacy. This Privacy Policy explains how we collect, use, disclose, and safeguard your information when you use our mobile application and services.

This Privacy Policy complies with the **Australian Privacy Act 1988** (Cth) and the **Australian Privacy Principles (APPs)**.

By using MenuLove, you agree to the collection and use of information in accordance with this policy.

---

## 2. Information We Collect

### 2.1 Personal Information

We collect the following personal information:

**For All Users:**
- Email address
- Name (optional)
- Location data (GPS coordinates)
- Device information (type, OS, unique identifiers)
- Usage data (videos watched, restaurants viewed, searches)
- IP address

**For Restaurant Partners:**
- Business name and ABN
- Contact details (email, phone)
- Business address
- Bank account details (for payments via Stripe)
- Payment and subscription information
- Content uploaded (videos, photos, menu items)

### 2.2 Automatically Collected Information

- App usage analytics
- Device identifiers
- Log data (IP address, browser type, pages visited)
- Cookies and similar technologies

### 2.3 Third-Party Data

We integrate with:
- **Google Places API** - Restaurant information and reviews
- **Stripe** - Payment processing
- **Supabase** - Database and authentication

---

## 3. How We Use Your Information

We use your information for:

### 3.1 Service Delivery
- Provide and maintain the MenuLove app
- Show nearby restaurants based on your location
- Process payments and subscriptions
- Authenticate users and partners
- Send service-related notifications

### 3.2 Improvement and Analytics
- Analyze app usage to improve features
- Understand user preferences
- Debug technical issues
- Develop new features

### 3.3 Marketing (with consent)
- Send promotional emails (you can opt-out anytime)
- Notify about new features or restaurants
- Partner newsletters

### 3.4 Legal Compliance
- Comply with Australian laws and regulations
- Respond to legal requests
- Prevent fraud and abuse
- Enforce our Terms of Service

---

## 4. Legal Basis for Processing (APPs Compliance)

Under the Australian Privacy Principles, we process your information based on:

- **Consent** - You provide explicit consent when signing up
- **Contract** - Processing necessary to provide our services
- **Legal Obligation** - Compliance with Australian law
- **Legitimate Interests** - Improving our services and preventing fraud

---

## 5. Data Sharing and Disclosure

### 5.1 We Share Data With:

**Service Providers:**
- **Stripe** - Payment processing (PCI-DSS compliant)
- **Supabase** - Database hosting (ISO 27001 certified)
- **Google** - Maps and Places API
- **Resend** - Email delivery

**Legal Requirements:**
- Law enforcement or government agencies (when legally required)
- Court orders or subpoenas
- Protection of our rights or safety

**Business Transfers:**
- In case of merger, acquisition, or sale of assets

### 5.2 We DO NOT:
- Sell your personal information to third parties
- Share your data for advertising purposes
- Use your location data for purposes other than showing nearby restaurants

---

## 6. Data Security

We implement industry-standard security measures:

- **Encryption** - Data encrypted in transit (TLS/SSL) and at rest
- **Authentication** - Secure login via Supabase Auth
- **Access Controls** - Limited employee access to personal data
- **Regular Audits** - Security reviews and updates
- **PCI Compliance** - Payment data handled by Stripe (PCI-DSS Level 1)

However, no method of transmission over the internet is 100% secure. We cannot guarantee absolute security.

---

## 7. Data Retention

We retain your information for:

- **Active Accounts** - As long as your account is active
- **Inactive Accounts** - 12 months after last activity, then deleted
- **Legal Requirements** - 7 years for financial records (Australian tax law)
- **Marketing** - Until you unsubscribe

You can request deletion of your data at any time (see Section 9).

---

## 8. International Data Transfers

Your data may be transferred to and stored on servers outside Australia:

- **Supabase** - Servers in USA (AWS)
- **Stripe** - Global infrastructure
- **Google Cloud** - Various locations

We ensure adequate safeguards are in place, including:
- Standard Contractual Clauses
- Privacy Shield frameworks
- Encryption and security measures

---

## 9. Your Rights (Australian Privacy Principles)

Under Australian law, you have the right to:

### 9.1 Access
Request a copy of your personal information

### 9.2 Correction
Request correction of inaccurate or incomplete data

### 9.3 Deletion
Request deletion of your personal information (subject to legal obligations)

### 9.4 Opt-Out
Unsubscribe from marketing emails

### 9.5 Complaint
Lodge a complaint with us or the Office of the Australian Information Commissioner (OAIC)

**To exercise your rights, contact us at:** contact@menulove.com.au

We will respond within 30 days.

---

## 10. Children's Privacy

MenuLove is not intended for users under 18 years of age. We do not knowingly collect personal information from children. If you believe we have collected information from a child, please contact us immediately.

---

## 11. Cookies and Tracking

We use cookies and similar technologies for:

- **Essential** - Authentication and security
- **Analytics** - Usage statistics (anonymized)
- **Preferences** - Remember your settings

You can disable cookies in your browser settings, but some features may not work properly.

---

## 12. Third-Party Links

Our app may contain links to third-party websites (e.g., restaurant websites, Google Maps). We are not responsible for their privacy practices. Please review their privacy policies.

---

## 13. Changes to This Privacy Policy

We may update this Privacy Policy from time to time. We will notify you of significant changes via:

- Email notification
- In-app notification
- Updated "Last Updated" date

Continued use of MenuLove after changes constitutes acceptance of the updated policy.

---

## 14. Contact Us

If you have questions or concerns about this Privacy Policy:

**Email:** contact@menulove.com.au  
**Address:** 16 Smith Street, Mooloolaba, Sunshine Coast, Queensland, Australia  
**ABN:** 33 234 268 637

---

## 15. Complaints

If you believe we have breached the Australian Privacy Principles, you can:

1. **Contact us directly** at contact@menulove.com.au
2. **Lodge a complaint** with the Office of the Australian Information Commissioner (OAIC):
   - Website: www.oaic.gov.au
   - Phone: 1300 363 992
   - Email: enquiries@oaic.gov.au

---

## 16. Data Protection Officer

For privacy-related inquiries, contact:

**Privacy Officer:** MenuLove Team  
**Email:** contact@menulove.com.au

---

**This Privacy Policy is governed by Australian law and complies with the Australian Privacy Act 1988 and the Australian Privacy Principles.**
